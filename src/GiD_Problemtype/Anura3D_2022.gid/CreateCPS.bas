...
$$SOLID_TRACTION
*GenData(solid_traction_A_type)  *GenData(solid_traction_A_multiplier_initial_[-])  *GenData(solid_traction_A_multiplier_final_[-])
$$SOLID_TRACTION_B
*GenData(solid_traction_B_type)  *GenData(solid_traction_B_multiplier_initial_[-])  *GenData(solid_traction_B_multiplier_final_[-])
$$LIQUID_PRESSURE
*GenData(liquid_pressure_A_type)  *GenData(liquid_pressure_A_multiplier_initial[-])  *GenData(liquid_pressure_A_multiplier_final_[-])
$$LIQUID_PRESSURE_B
*GenData(liquid_pressure_B_type)  *GenData(liquid_pressure_B_multiplier_initial_[-])  *GenData(liquid_pressure_B_multiplier_final_[-])
$$PRESCRIBED_VELOCITY
*GenData(prescribed_velocity_type)  *GenData(velocity_multiplier_initial_[-])  *GenData(velocity_multiplier_final_[-])
$$HYDRAULIC_HEAD
*GenData(hydraulic_head_type) *GenData(hydraulic_head_value)
$$APPLY_SEEPAGE_FACE
*GenData(seepage_ID)
$$APPLY_INFILTRATION
*GenData(infiltration_ID)
...
$$APPLY_SMOOTHENING_LIQUID_PRESSURE_INCREMENT
*GenData(liquid_pressure_smoothing_ID)
$$CONTACT_FORMULATION
*GenData(contact_ID)
...
*if((GenData(max_suction_at_soil_surface_ID,int)==1))
$$K0_MAX_SUCTION
*GenData(suction_[kPa]__)
*end if
*end if
*##### excavation #####
$$EXCAVATION
...
*if((GenData(visualization_option_ID,int)==100))
Paraview
*end if
*if((GenData(visualization_option_ID,int)==101))
GiD-ASCII
*end if
*if((GenData(visualization_option_ID,int)==102))
GiD-Binary
*end if
*if((GenData(visualization_option_ID,int)==103))
Paraview-GiD
*end if
*##### OUTPUT DATA #####
