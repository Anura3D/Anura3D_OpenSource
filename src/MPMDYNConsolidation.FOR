      ...
      contains
        ...
        implicit none
          ...
          if (NFORMULATION==1) then !mixture
              if ((CalParams%NumberOfPhases==2).or.(CalParams%NumberOfPhases==3)) then
                ...
                allocate(ExtLoadWaterTotal(Counters%N, Counters%NEntity), stat = IError)
              else
                ...
                allocate(ExtLoadWaterTotal(1, 1), stat = IError)
              end if
          else !constituents
              allocate(LumpedMassWater(Counters%N, Counters%NEntity), stat = IError)
              allocate(LumpedMassNWater(Counters%N, Counters%NEntity), stat = IError)
        ...
        implicit none
          ...
          if (Counters%NLoadedElementSidesWaterNodes > 0) then
            ExtLoadWater = ExtLoadWater + ExtLoadWaterTotal * CalParams%Multipliers%WaterACurrent
          end if
        ...
        implicit none
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, IPart, ParticleIndex, ParticleVelocity, ParticleMass,
          ! !$omp&         iEntity, INode, NodeID, Nix)
          ! !$omp& reduction(+ : Momentum)
        ...
        implicit none
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, IDof, NodAcc,
          ! !$omp&         IPart, ParticleIndex, ParticleVelocity, ParticleMass,
          ! !$omp&         ParticleShape, ParticleIncrementalVelocity,
          ! !$omp&         ParticleAcceleration, INode, IEntity)
          ! !$omp& reduction(+ : Momentum)
          do IAEl = 1, Counters%NAEl                                      ! Loop over all elements
            IEl = ActiveElement(IAEl)
        ...
        !**********************************************************************
          ...
          ! !$omp parallel do
          ! !$omp& private(IAElement, IElement, NElemPart, IParticle, ParticleIndex,
          ! !$omp&         BMatrixDeformed, DetJac, iEntity, iEntityDefault, 
          ! !$omp&         DUTotEnt, Eps)
        ...
        implicit none
         ...
         ! local variables
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAElement, IElement, PartGravity, IPart, ParticleIndex,
          ! !$omp&         iEntity, INode, NodeID, GlobDof)
          ! !$omp& reduction(+ : GravityLoadWaterLoc)
          do IAElement = 1, Counters%NAEl ! Loop over all elements
           IElement = ActiveElement(IAElement)
        ...
        implicit none
          ...
          do IAEl = 1, Counters%NAEl                                    ! Loop over all elements
            ...
            do IPart = 1, NPartEle(IEl)                                 ! Loop over all particles in element
             ...
             if(trim(MatParams(MaterialID)%MATERIALTYPE)=='2-phase'.or.MatParams(MaterialID)%MaterialPhases=='2-phase') then
                 ConstDensityLiquid = (MatParams(MaterialID)%DensityLiquid/1000)
                 LiquidViscosity = MatParams(MaterialID)%ViscosityLiquid
                 IntrinsicPermeabilityLiquid = MatParams(MaterialID)%IntrinsicPermeabilityLiquid
             else if(trim(MatParams(MaterialID)%MATERIALTYPE)=='1-phase-liquid'.or.MatParams(MaterialID)%MaterialPhases=='1-phase-liquid') then
                 ConstDensityLiquid = (MatParams(MaterialID)%DensityLiquid/1000)
                 LiquidViscosity = MatParams(MaterialID)%ViscosityLiquid 
             else if(trim(MatParams(MaterialID)%MATERIALTYPE)=='1-phase-solid'.or.MatParams(MaterialID)%MaterialPhases=='1-phase-solid') then
                 IntrinsicPermeabilityLiquid = MatParams(MaterialID)%IntrinsicPermeabilityLiquid
             end if
        ...
        implicit none
          ...
          integer(INTEGER_TYPE) :: IPart, IAEl, ParticleIndex, NodeID
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, IPart, ParticleIndex, PartLoad, PartGravity,
          ! !$omp&        PartGravityMix, INode, NodeID, IDof, 
          ! !$omp&        NElemPart, B, Det, WTN, Int, IntGlo, S, iEntityID, nn,
          ! !$omp&        TotIntWeightLiqMP, SW)
          ! !$omp& reduction(+ : ExtLoadWaterLoc) 
          ! !$omp& reduction(+ : IntLoadWaterLoc) 
          ! !$omp& reduction(+ : GravityLoadWaterLoc) 
          ! !$omp& reduction(+ : GravityLoadMixtureLoc) 
          do IAEl = 1, Counters%NAEl
            IEl = ActiveElement(IAEl)
              ...
              do IPart = 1, NPartEle(IEl) ! Loop over all particles in element
                ...
                do INode = 1, ELEMENTNODES 
                  ...
                  if(Counters%NLoadedElementSidesWaterMatPoints > 0) then 
                    PartLoad = ShapeValuesArray(ParticleIndex,INode) * Particles(ParticleIndex)%FExtWater
                    do I = 1, NVECTOR
                      ExtLoadWaterLoc(IDof+I, iEntityID) =  ExtLoadWaterLoc(IDof+I, iEntityID) + PartLoad(I) * CalParams%Multipliers%WaterACurrent
                    end do
                  end if
...
!------------------------------------ END INTEGRATION POINT LOOP -----------------------------  
        ...
        implicit none
          ...
          ! !$omp parallel do
          ! !$omp& private(I, J, IAEl, IEl, ParticleIndex, INode,  
          ! !$omp&        NElemPart, B, Det, WTN, Int, IntGlo, S, iEntityID, nn,
          ! !$omp&        IDof, TotIntWeightLiqMP, SW)
          ! !$omp& reduction(+ : IntLoadWaterLoc) 
          do IAEl = 1, Counters%NAEl
            IEl = ActiveElement(IAEl)
      ...
      subroutine ConsolidationExtForces(ExtLoadWaterLoc, &
        ...
        implicit none
          ...
          ! Local variables
          integer(INTEGER_TYPE) :: I, IEl, iNode, IPart, IAEl, ParticleIndex, NodeID, IDof, iEntity
          real(REAL_TYPE), dimension(NVECTOR) :: PartLoad, PartGravity, PartGravityMix
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, IPart, ParticleIndex, PartLoad, PartGravity,
          ! !$omp&        PartGravityMix, iEntity, INode, NodeID, IDof)
          ! !$omp& reduction(+ : ExtLoadWaterLoc)
          ! !$omp& reduction(+ : GravityLoadWaterLoc) 
          ! !$omp& reduction(+ : GravityLoadMixtureLoc) 
          do IAEl = 1, Counters%NAEl
              ...
              do IPart = 1, NPartEle(IEl) ! Loop over all particles in element
                ...
                do INode = 1, ELEMENTNODES 
                  ...
                  if(Counters%NLoadedElementSidesWaterMatPoints > 0) then 
                    PartLoad = ShapeValuesArray(ParticleIndex,INode) * Particles(ParticleIndex)%FExtWater
                    do I = 1, NVECTOR
                      ExtLoadWaterLoc(IDof+I, iEntity) =  ExtLoadWaterLoc(IDof+I, iEntity) + PartLoad(I) * CalParams%Multipliers%WaterACurrent
                    end do
                  end if
      ...
      subroutine CorrectWaterForPrescribedVelocity(InterfaceNodes,InterfaceNodeNormals,PrescrVelocity)
        ...
        implicit none
          ...
          if (.not.InterfaceNodes(nn)) CYCLE
           ...
           do J = 1, NVECTOR
              VelocityValues(J) = TotalVelocityWater(IDoF + J, SOIL_ENTITY)
           ...
           do J = 1, NVECTOR
              TotalVelocityWater(IDoF + J, SOIL_ENTITY) = VelocityValues(J)
              AccelerationWater(IDoF + J, SOIL_ENTITY) =  (TotalVelocityWater(IDoF + J, SOIL_ENTITY) - TotalVelocityWaterPrevious(IDoF + J, SOIL_ENTITY)) /  CalParams%TimeIncrement
           end do
           end do
      ...
      subroutine ConsolidationForcesPorosity(ExtLoadWaterPorosityLoc, &
        ...
        implicit none
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, IPart, ParticleIndex, PartLoad, PartGravity,
          ! !$omp&       PartPorosity, PartDegreeSat,
          ! !$omp&        iEntity, INode, NodeID, GlobDof, 
          ! !$omp&        NElemPart, B, Det, WTN, Int, IntGlo, S, iEntityID, nn, IDof)
          ! !$omp& reduction(+ : ExtLoadWaterPorosityLoc) 
          ! !$omp& reduction(+ : IntLoadWaterPorosityLoc) 
          ! !$omp& reduction(+ : GravityLoadWaterPorosityLoc)  
          do IAEl = 1, Counters%NAEl
            IEl = ActiveElement(IAEl)
...
!//////////////////// External force calculation //////////////////////// 
            do IPart = 1, NPartEle(IEl) ! Loop over all particles in element
              ...
              do INode = 1, ELEMENTNODES 
                ...
                if(Counters%NLoadedElementSidesWaterMatPoints > 0) then 
                  PartLoad = ShapeValuesArray(ParticleIndex,INode) * Particles(ParticleIndex)%FExtWater * PartPorosity * PartDegreeSat
                  do I = 1, NVECTOR
                    ExtLoadWaterPorosityLoc(IDof+I, iEntity) = ExtLoadWaterPorosityLoc(IDof+I, iEntity) + PartLoad(I) * CalParams%Multipliers%WaterACurrent
                  end do
                end if
...
!//////////////////// Internal force calculation ////////////////////////
      ...
      subroutine ConsolidationIntForcesPorosity(IntLoadWaterPorosityLoc)
        ...
        implicit none
          ...
          ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, INode,  
          ! !$omp&       PartDegreeSat, PartPorosity,
          ! !$omp&        NElemPart, B, Det, WTN, Int, IntGlo, S, iEntityID, nn, IDof)
          ! !$omp& reduction(+ : IntLoadWaterPorosityLoc)
          do IAEl = 1, Counters%NAEl
            IEl = ActiveElement(IAEl)
       ...
       subroutine ConsolidationExtForcesPorosity(ExtLoadWaterPorosityLoc, GravityLoadWaterPorosityLoc)
         ...
         implicit none
           ...
           ! !$omp parallel do
          ! !$omp& private(I, IAEl, IEl, IPart, ParticleIndex, PartLoad, PartGravity,PartDegreeSat,PartPorosity,
          ! !$omp&         iEntity, INode, NodeID, IDof, 
          ! !$omp&        NElemPart, Int, IntGlo, nn)
          ! !$omp& reduction(+ : ExtLoadWaterPorosityLoc) 
          ! !$omp& reduction(+ : IntLoadWaterPorosityLoc) 
          ! !$omp& reduction(+ : GravityLoadWaterPorosityLoc)  
           do IAEl = 1, Counters%NAEl
      ...
      end module ModMPMDYNConsolidation