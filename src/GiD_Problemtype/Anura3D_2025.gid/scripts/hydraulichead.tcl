#######################################################
# HYDRAULIC HEAD IN TIME
#######################################################

package require fulltktree 
package require base64

namespace eval HydraulicHead {                   
}

proc HydraulicHead::create_table { domNode } {
    global Priv    
    ...
    set w [dialogwin_snit $w -title [= "Hydraulic head in time"] \
    ...
    set tree [fulltktree $f1.left -columns $columns -height 420 -width 100 \
            ...
            -contextualhandler [list HydraulicHead::contextualmenu $w] \
            -deletehandler [list HydraulicHead::delete $w $f1.left selection]]
    ...
    ttk::button $f11.b1 -text [_ "Add at end"] -command \
        [list HydraulicHead::add $w $tree end $f11.data] -width 20
    ttk::button $f11.b2 -text [_ "Add before"] -command \
        [list HydraulicHead::add $w $tree prev $f11.data] -width 20
    ttk::button $f11.b3 -text [_ "Clear all"] -command \
        [list HydraulicHead::delete $w $tree all] -width 20
    ...
    HydraulicHead::actualize_graph $w $tree
    ...
    while 1 {
        switch -- $action {             
            ...
            1 {
                ...
                HydraulicHead::save_data $w $tree $domNode 
                catch { destroy $w }
                return
            }
        }        
...
proc HydraulicHead::add { w tree where f } {
...
proc HydraulicHead::actualize_graph { w tree } {
...
proc HydraulicHead::contextualmenu { w tree items x y } {

    catch { destroy $tree._cmenu }
    set menu [menu $tree._cmenu -tearoff 0]

    $menu add command -label [_ "Select all"] \
        -command [list HydraulicHead::cutcopypaste $w $tree selectall]
    $menu add separator
    $menu add command -label [_ "Copy"] \
        -command [list HydraulicHead::cutcopypaste $w $tree copy]
    $menu add command -label [_ "Paste"] \
        -command [list HydraulicHead::cutcopypaste $w $tree paste]
    $menu add command -label [_ "Paste end"] \
        -command [list HydraulicHead::cutcopypaste $w $tree paste_end]
    $menu add separator

    $menu add command -label [_ "Delete"] \
        -command [list HydraulicHead::delete $w $tree selection]
    
    $menu add command -label [_ "Delete all"] \
        -command [list HydraulicHead::delete $w $tree all]
    
    tk_popup $menu $x $y
}

proc HydraulicHead::cutcopypaste { w tree what } {
    ...
    switch $what {
        ...
        paste {
            if { [$tree item count] <= 1 } {
                ...
                return [HydraulicHead::cutcopypaste $w $tree paste_end]
            }
...
proc HydraulicHead::value_type { w f button } {
...
proc HydraulicHead::delete { w tree what args } {
...
proc HydraulicHead::save_data { w tree domNode } {
    ...
    if { $containerNode == "" } {
        $domNode appendChildTag container \
            [list attributes() n "hydraulic_head_data" \
                pn "Hydraulic head data" state hidden]
        set containerNode [$domNode selectNodes {container}]
    } else {
        foreach iNode [$containerNode selectNodes {value}] {
            $iNode delete
