    module ModHyperbolicElasticity
    contains 
    
    Subroutine ESM_LINEAR_HYP(NPT,NOEL,IDSET,STRESS,EUNLOADING,PLASTICMULTIPLIER, &
...
!      INCLUDE 'ABA_PARAM.INC'
      ...
      PROPS(NPROPS),COORDS(3),DROT(3,3),DFGRD0(3,3),DFGRD1(3,3)
        ...
        ! Contents of PROPS(2)
        !  1 : G_o     Initial shear modulus
        !  2 : xq_ult  Ultimate deviatoric stress
        !  3 : ENU     Poisson's ratio

        G_o = PROPS(1)   
        xq_ult = PROPS(2)
        ENU = PROPS(3)
        
        ! Compute volumetric (mean) strain and deviatoric strain components in Voigt format
        one = 1.0d0
        two = 2.0d0
        ! mean (volumetric) strain
        eps_mean = (STRAN(1) + STRAN(2) + STRAN(3)) / 3.0d0
        ! deviatoric strain components (Voigt): subtract mean from normal components
        dev_eps1 = STRAN(1) - eps_mean
        dev_eps2 = STRAN(2) - eps_mean
        dev_eps3 = STRAN(3) - eps_mean
        dev_eps4 = STRAN(4)
        if (NTENS == 6) then
          dev_eps5 = STRAN(5)
          dev_eps6 = STRAN(6)
        ...
        ! compute second invariant of deviatoric strain: dev:dev in Voigt notation
        ! Note: shear components in STRAN are engineering shear (gamma_xy), so when
        ! computing inner product use factor 1/2 for off-diagonals if needed. For a
        ! common convention (Voigt with shear engineering strains) the deviatoric
        ! norm squared = dev_eps1**2 + dev_eps2**2 + dev_eps3**2 + 0.5*(dev_eps4**2+dev_eps5**2+dev_eps6**2)
        dev_dot_dev = dev_eps1*dev_eps1 + dev_eps2*dev_eps2 + dev_eps3*dev_eps3 + 0.5d0*(dev_eps4*dev_eps4 + dev_eps5*dev_eps5 + dev_eps6*dev_eps6)
        ! equivalent deviatoric strain (von Mises-like) sqrt(2/3 * dev:dev)
        equiv_dev_strain = sqrt( (2.0d0/3.0d0) * dev_dot_dev )
        ! calculate elastic stress increment (DSigE = elastic stiffness D * strain increment DEps)

        !Compute tangent shear modulus based on hyperbolic model
        G = (1.0d0/(9.0*G_o)) / (( (1.0d0/(3.0*G_o)) + (equiv_dev_strain / xq_ult) )**2)
  ...
  FAC = two * G / ( one - two * ENU )
    ...
    end module ModHyperbolicElasticity 