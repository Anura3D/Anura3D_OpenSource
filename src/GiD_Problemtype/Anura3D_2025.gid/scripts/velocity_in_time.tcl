...
namespace eval VelocityInTime {                   
}

proc VelocityInTime::create_table { domNode } {
    global Priv    
    ...
    set xp {container[@n="General_data"]/container[@n="units"]/value[@n="units_time"]}
    set time_unitNode [$root selectNodes $xp]
    set time_unit [$time_unitNode @v]
    ...
    set w [dialogwin_snit $w -title [= "Velocity in time"] \
    ...
    set y_units "-"
    set columns [list \
            [list 10 "t \[$time_unit\]" left text 0 ] \
            [list 30 "multiplier \[$y_units\]" left text 0] \
            ]
    
    set f11 [ttk::labelframe $f1.f1 -text [_ "Values entry"]]
    set fgraph [ttk::labelframe $f11.fgraph -text [_ "Graph"]]
    
    $w set_uservar_value fgraph $fgraph 
    
    set tree [fulltktree $f1.left -columns $columns -height 420 -width 140 \
            -expand 1 -bd 1 \
            -selectmode extended -expand 1 -bd 1 -relief solid \
            -showlines 0 -sensitive_cols all \
            -contextualhandler [list VelocityInTime::contextualmenu $w] \
            -deletehandler [list VelocityInTime::delete $w $f1.left selection]]
        
    $tree notify bind [$tree givetreectrl] <Header-invoke> ""
    $w set_uservar_value tree $tree  
    
    ttk::label $f11.label_x -text "t \[$time_unit\]"
    ttk::entry $f11.entry_x -textvariable \
        [$w give_uservar entry_x] -width 22
    $w set_uservar_value entry_x "0.0"
    
    ttk::label $f11.label_y -text "multiplier \[$y_units\]"
    ...
    ttk::button $f11.b1 -text [_ "Add at end"] -command \
        [list VelocityInTime::add $w $tree end $f11.data] -width 20
    ttk::button $f11.b2 -text [_ "Add before"] -command \
        [list VelocityInTime::add $w $tree prev $f11.data] -width 20
    ttk::button $f11.b3 -text [_ "Clear all"] -command \
        [list VelocityInTime::delete $w $tree all] -width 20
    ...
    VelocityInTime::actualize_graph $w $tree
            ...
            1 {
                ...
                if { ![llength [$tree item children 0]] } {
                    snit_messageBox -parent $w -message \
                        [= "There are no points to define the velocity in time profile"]
                } elseif { [llength [$tree item children 0]] < 2 } {
                    snit_messageBox -parent $w -message \
                        [= "There are not enough points to define the velocity in time profile"]
                }
                VelocityInTime::save_data $w $tree $domNode
                set parentNode [$domNode selectNodes {..}]                
                if { [$parentNode nodeName] eq "group" } {
                    VelocityInTime::UpdateVelocityIntime $parentNode   
                }
                catch { destroy $w }
                return
            }
...
proc VelocityInTime::add { w tree where f } {
...
proc VelocityInTime::actualize_graph { w tree } {
...
proc VelocityInTime::contextualmenu { w tree items x y } {

    catch { destroy $tree._cmenu }
    set menu [menu $tree._cmenu -tearoff 0]

    $menu add command -label [_ "Select all"] \
        -command [list VelocityInTime::cutcopypaste $w $tree selectall]
    $menu add separator
    $menu add command -label [_ "Copy"] \
        -command [list VelocityInTime::cutcopypaste $w $tree copy]
    $menu add command -label [_ "Paste"] \
        -command [list VelocityInTime::cutcopypaste $w $tree paste]
    $menu add command -label [_ "Paste end"] \
        -command [list VelocityInTime::cutcopypaste $w $tree paste_end]
    $menu add separator

    $menu add command -label [_ "Delete"] \
        -command [list VelocityInTime::delete $w $tree selection]
    
    $menu add command -label [_ "Delete all"] \
        -command [list VelocityInTime::delete $w $tree all]
    
    tk_popup $menu $x $y
}

proc VelocityInTime::cutcopypaste { w tree what } {
    ...
    switch $what {
        ...
        paste {
            if { [$tree item count] <= 1 } {
                ...
                return [VelocityInTime::cutcopypaste $w $tree paste_end]
            }
...
proc VelocityInTime::value_type { w f button } {
...
proc VelocityInTime::delete { w tree what args } {
...
proc VelocityInTime::save_data { w tree domNode } {
    ...
    if { $containerNode == "" } {
        $domNode appendChildTag container \
            [list attributes() n "velocity_data" \
                pn "velocity data" state hidden]
        set containerNode [$domNode selectNodes {container}]
    } else {
        foreach iNode [$containerNode selectNodes {value}] {
            $iNode delete
...
proc VelocityInTime::UpdateVelocityIntime { domNode } {
    if { [$domNode nodeName] eq "group"} {
        set groupNode $domNode       
    } else {            
        set groupNodeList [$domNode selectNodes {group}]
        set groupNode [lindex $groupNodeList end]
    }
    
    # NODAL PRESCIBED VELOCITY & MATERIAL POINT PRESCRIBED VELOCITY
    
    #  X_velocity_[m/s]
    set X_velocityNode [$domNode selectNodes {value[@n="X_velocity_[m/s]"]}]
    set velocity_dataNode [$X_velocityNode selectNodes {container[@n="velocity_data"]}]
    set X_velocity_groupNode [$groupNode selectNodes {value[@n="X_velocity_[m/s]"]}]
    if { $velocity_dataNode != ""} {       
        set aux [$velocity_dataNode cloneNode -deep]
        $X_velocity_groupNode appendChild $aux
        $X_velocity_groupNode setAttribute v 1.0
        $velocity_dataNode delete
    } else {
        $X_velocity_groupNode setAttribute v 0.0
    }
    
    # Y_velocity_[m/s]
    set Y_velocityNode [$domNode selectNodes {value[@n="Y_velocity_[m/s]"]}]
    set velocity_dataNode [$Y_velocityNode selectNodes {container[@n="velocity_data"]}]
    set Y_velocity_groupNode [$groupNode selectNodes {value[@n="Y_velocity_[m/s]"]}]
    if { $velocity_dataNode != ""} {      
        set aux [$velocity_dataNode cloneNode -deep]
        $Y_velocity_groupNode appendChild $aux
        $Y_velocity_groupNode setAttribute v 1.0
        $velocity_dataNode delete
    } else {
        $Y_velocity_groupNode setAttribute v 0.0
    }
    
    # Z_velocity_[m/s]
    set Z_velocityNode [$domNode selectNodes {value[@n="Z_velocity_[m/s]"]}]
    set velocity_dataNode [$Z_velocityNode selectNodes {container[@n="velocity_data"]}]
    set Z_velocity_groupNode [$groupNode selectNodes {value[@n="Z_velocity_[m/s]"]}]
    if { $velocity_dataNode != ""} {        
        set aux [$velocity_dataNode cloneNode -deep]
        $Z_velocity_groupNode appendChild $aux
        $Z_velocity_groupNode setAttribute v 1.0
        $velocity_dataNode delete
    } else {
        $Z_velocity_groupNode setAttribute v 0.0
    }   
